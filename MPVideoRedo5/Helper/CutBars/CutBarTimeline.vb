    ...
    Public Overridable Property Separators() As Integer
        ...
        Set(ByVal value As Integer)
            If Me.m_separators <> value Then
                ...
                Invalidate()
                OnSeparatorsChanged(New CutBarProgressEventArgs)
            End If
        End Set
    End Property
        ...
        For i As Integer = 0 To StartCutValues.Count - 1
            ...
            If EndCutValues(i) - StartCutValues(i) <> 0 Then
                ...
                Using brush As New LinearGradientBrush(rect, LightFillColor, DarkFillColor, LinearGradientMode.Vertical)
                    Dim xPos As Single = rect.X + CSng(rect.Width) * CSng(StartCutValues(i)) / 100.0F
                    Dim yWidth As Single = CSng(rect.Width) * CSng(EndCutValues(i) - StartCutValues(i)) / 100.0F
                    Dim clip As New RectangleF(xPos, rect.Y, yWidth, rect.Height)
                    g.SetClip(clip)
                    g.FillPath(brush, path)
                    g.ResetClip()
                End Using
