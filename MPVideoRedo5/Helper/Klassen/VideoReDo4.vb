...
Public Class VideoReDo
    ...
    Public Sub AddMarker(ByVal Pos As Long)
        'If Pos >= 0 And Pos <= VideoDuration Then
        mCutMarker.Add(Pos)
        ...
        If IO.File.Exists(VideoFile) Then
            ...
            If mFile.Extension.ToLower = ".ts" Or mFile.Extension.ToLower = ".mpeg" Or mFile.Extension = ".mpg" Then
                If mFile.Length > 10000000 Then
                    ...
                    End If
                    VideoDuration = VRD.GetProgramDuration * 1000
                    If VRD.setcutmode(1) = 1 Then
                        Logger.DebugM("Cutmode von VideoRedo erfolgreich auf CutMode gestellt")
                    Else
                        Logger.Warn("Cutmode von VideoRedo konnte nicht umgestellt werden.")
                    End If
                    Return True
                Else
    ...
    Public Function SeekToTime(ByVal Millisekunden, Optional BarPosition = 0) As Boolean
        ...
        'Logger.DebugM("TemporarySeekTime: {0}", Millisekunden)
        ...
        'Logger.DebugM("ERGEBNISS: {0}", Me.GetCursorTime = sobj.SeekTime)
        Return True
    End Function

    Public Function MakeScreenshotClipboard(ByVal MSek As Long, Optional ByVal Quali As ScreenshotQuality = ScreenshotQuality.poor, Optional BarPosition As Integer = 0) As Image
        ...
        Return bmp2
    End Function
    Public Function MakeScreenshotFile(ByVal MSek As Integer, ByVal FileName As String, Optional ByVal Quali As ScreenshotQuality = ScreenshotQuality.poor) As Boolean
    ...
    End Function
    Public Function MakeScreenshotImageData(ByVal MSek As Integer, ByVal FileName As String, Optional ByVal Quali As ScreenshotQuality = ScreenshotQuality.poor) As Image
        If VRD.SeekToTimeMsec(MSek) = True Then
    ...
    Public Function SaveFileAsEx(ByVal Filename As String, Optional ByVal OutputType As VideoSaveFormat = 1) As Boolean
        ...
        If Left(Me.ReDoVersion, 1) = 4 Then
            ...
            Dim temp As Object = VRD.FileSaveProfile(Filename, SavingProfile)
...
#End Region
    ...
    Private Sub SeekInBackground(SeekObject As Object)
        Try
            Dim sObj As SeekingObject = DirectCast(SeekObject, SeekingObject)
            Logger.DebugM("Trying to seek position: {0}", sObj.SeekTime.ToString)
            ...
            Logger.DebugM("SeekToTimeMsec aus dem SeekInBackground-Thread heraus war erfolgreich? " & result.ToString)
            Try
                seekTr.Abort()
