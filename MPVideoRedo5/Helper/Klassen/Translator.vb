...
#Region "Constructor"
    ...
    Shared Sub New()
        ...
        _path = Config.GetSubFolder(Config.Dir.Language, "MPVideoRedo5")
    ...
    Private Shared Function LoadTranslations() As Integer
        ...
        Catch e As Exception
            ...
            If e.[GetType]() Is GetType(FileNotFoundException) Then
                Logger.Warn("Cannot find translation file {0}.  Falling back to German", langPath)
            Else
                Logger.[Error]("Error in translation xml file: {0}. Falling back to German", Lang)
                Logger.[Error](e)
            End If
        ...
        For Each fi As FieldInfo In fieldInfos
            ...
            Else
                Logger.Warn("Translation not found for field: {0}.  Using hard-coded German default.", fi.Name)
            End If
        Next
...
#Region "Public Methods"
    ...
    Public Shared Sub TranslateSkin()
        Logger.Info("Translating skin")
        For Each name As String In Strings.Keys
            SetProperty("#MPVideoRedo5.Translation." & name, Translator.Strings(name))
        Next
    End Sub
